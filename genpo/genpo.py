...
def genpo_full_zeroes(zeroes_multiplicity: list[int], highest_coff=None):
    """
    ...
    gens.raise_if_not_valid_multiplicities(multiplicities=zeroes_multiplicity)

    coff = gens.generate_coefficients(zeroes_multiplicity)

    ops.multiply(coff, highest_coff if highest_coff !=
                 None else gens.generate_high_coff())
    return coff


def genpo_full_zeroes_count(grade: int, zeroes_count: int = None, show_zeroes=False):
    """
    ...
    zeroes = gens.generate_zeroes(zeroes_count)
    zeroes_multiplicity = gens.span_random(zeroes, grade)

    coff = genpo_full_zeroes(zeroes_multiplicity)
...
def genpo_full_zeroes_multiplicities(multiplicities: list[int], show_zeroes=False):

    zeroes_multiplicity = gens.generate_zeroes_with_multiplicity(
        multiplicities)
...
def genpo_1(multiplicities: list[int] = None, grade: int = None, show_zeroes=False):
    ...
    gens.raise_if_not_valid_multiplicities(
    ...
    zeroes_with_multiplicity = gens.generate_zeroes_with_multiplicity(
        multiplicities)
    coff_with_zeroes = genpo_full_zeroes(
    ...
    coffs_without_zeroes = [gens.genpo_2nd_grade_no_zeroes()
...
def genpo_2(min_grade: int = None, max_grade: int = None, min_zeroes: int = None, max_zeroes: int = None, min_multeplicity: int = None, max_multeplicity: int = None):
    ...
    if zeroes_count > 0:
        multiplicities = gens.generate_multiplicities(
            zeroes_count, min_grade, max_grade, min_multeplicity, max_multeplicity)
        zeroes = gens.generate_zeroes_with_multiplicity(multiplicities)
        pol_with_zeroes = gens.generate_coefficients(zeroes)
    ...
    if grade_without_zeroes > 0:
        second_grade_pols_without_zeroes = [
            gens.genpo_2nd_grade_no_zeroes() for _ in range(int(grade_without_zeroes / 2))]
    ...
    ops.multiply(pol, gens.generate_high_coff())
...
def main_full_zeroes(pols: int = 1):
    for _ in range(pols):
        ...
        coff = genpo_full_zeroes_count(pol_grade, zeroes_count=zeroes_count)
